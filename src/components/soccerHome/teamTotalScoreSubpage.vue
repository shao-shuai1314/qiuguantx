  ...
  <div class="gWidth">
    ...
    <el-card>
      <el-tabs type="border-card"
        ...
        <el-tab-pane label="全场大小球盘"
                     name="all"></el-tab-pane>
        <el-tab-pane label="主场大小球盘"
                     name="home"></el-tab-pane>
        <el-tab-pane label="客场大小球盘"
        ...
        <el-table-column align="center"
          ...
          <el-table-column align="center"
            ...
            <template slot-scope="scope">
              <span v-if="scope.row.result == '大'"
                    style="font-weight: 900;"
                    :style="{'color':scope.row.result == '大'?'rgb(255, 0, 0)':''}">{{scope.row.result}}</span>
              <span v-else-if="scope.row.result == '小'"
                    style="font-weight: 900;"
                    :style="{'color':scope.row.result == '小'?'rgb(51, 102, 255)':''}">{{scope.row.result}}</span>
              <span v-else
  ...
  methods: {
    ...
    async tableDataList (styV) {
      ...
      if (!this.tableDatas.length) {
        ...
        const res = await this.$http.get(`odds/teamTotalScore/`, { params: obj });
        if (res.status !== 200) return console.log('大小球获取失败');

        res.data.teamLetgoal.forEach(item => {
          if (item.homeScore + item.guestScore > item.goal_real) {
            item.result = '大'
          } else if (item.homeScore + item.guestScore < item.goal_real) {
            item.result = '小'
          } else {
            item.result = '走'
      ...
      if (styV) {
        if (styV == 'home') {
          this.tit = this.matchSeason + `(` + this.teamName + `)` + '主场大小球盘路'
          this.tableData = this.tableDatas.filter(item => {
            return item.hometeamID === this.teamID

          })
        } else if (styV == 'guest') {
          this.tit = this.matchSeason + `(` + this.teamName + `)` + '客场大小球盘路'
          this.tableData = this.tableDatas.filter(item => {
            return item.guestteamID === this.teamID
          })
        } else {
          this.tit = this.matchSeason + `(` + this.teamName + `)` + '全场大小球盘路'

<template>
  <div class="gWidth">
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>让球盘统计</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- 左边 -->
    ...
    <el-card style="width:942px;"
      ...
      <el-table :data="tableData"
        ...
        <el-table-column prop="up"
                         align="center"
                         label="上盘"
                         width="60">
        </el-table-column>
        <el-table-column prop="flatpan"
                         align="center"
                         label="平盘"
                         width="60">
        </el-table-column>
        <el-table-column prop="down"
                         align="center"
                         label="下盘"
                         width="60">
        </el-table-column>
        ...
        <el-table-column align="center"
                         ...
                         width="60">
          <template slot-scope="scope">
            {{(scope.row.win/scope.row.count*100).toFixed(2)}}%
          </template>
        </el-table-column>
        <el-table-column align="center"
                         label="走(%)"
        ...
        <el-table-column align="center"
                         ...
                         width="60">
          <template slot-scope="scope">
            {{(scope.row.fail/scope.row.count*100).toFixed(2)}}%
          </template>
        </el-table-column>
        <el-table-column align="center"
                         label="详情"
          ...
          <template slot-scope="scope">
            <router-link target="_blank"
                         :to="{name:'concedePointsSubpage', query: {teamID:scope.row.teamID,sclassID,matchSeason}}">详情</router-link>
          </template>
        </el-table-column>
...
export default {
  ...
  created () {
    ...
    document.title = `${this.matchSeason} - ${this.datas[0]} -  让盘球统计`
  },
  methods: {
    ...
    async tableDataList (sty) {
      if (!this.tableData.length) {
        if (sessionStorage.getItem('subsClassID')) {
          const res = await this.$http.get(`soccer/sclass/${this.$route.params.sclassID}/teamLetgoal/`, { params: { matchSeason: sessionStorage.getItem('matchSeason'), subsClassID: sessionStorage.getItem('subsClassID') } });
          if (res.status !== 200) return console.log('让盘球获取失败');
          this.tableDatas = res.data
        } else {
          const res = await this.$http.get(`soccer/sclass/${this.$route.params.sclassID}/teamLetgoal/`, { params: { matchSeason: sessionStorage.getItem('matchSeason') } });
          if (res.status !== 200) return console.log('让盘球获取失败');
          this.tableDatas = res.data
        }
      }
